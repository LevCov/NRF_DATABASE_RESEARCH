...
void RedisInterface::create(const StringView &key, const StringView &field,
...
void RedisInterface::update(const StringView &key, const StringView &field,
                            const StringView &dataField) {
...
std::vector<OptionalString> RedisInterface::find(
  ...
  for (const auto &key : keys) {
    if ((*this).hget(*key, "nfType") == nfTypeSearch) match_keys.push_back(key);
  }
